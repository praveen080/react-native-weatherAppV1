...
const MyComponent = () => {
  ...
  const saveDataToCache = async (text) => {
    setInputData(text); // Update local state as the user types
    try {
      await AsyncStorage.setItem("userInput", text);
  ...
  return (
    <View style={styles.container}>
      <TextInput
        ...
        value={inputData}
        onChangeText={saveDataToCache} // Save data to cache and update local state
      />
